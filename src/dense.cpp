...
void DenseLayer::backward(const MatrixXd &dvalues)
{
    try
    {
        ...
        if (weight_regularizer_l1 > 0)
        {
            ...
            dL1 = (weights.array() < 0).select(Eigen::MatrixXd::Constant(weights.rows(), weights.cols(), -1.), Eigen::MatrixXd::Constant(weights.rows(), weights.cols(), 1.));
        ...
        if (bias_regularizer_l1 > 0)
        {
            ...
            dL1 = (biases.array() < 0).select(Eigen::RowVectorXd::Constant(biases.size(), -1), Eigen::RowVectorXd::Constant(biases.size(), 1));
